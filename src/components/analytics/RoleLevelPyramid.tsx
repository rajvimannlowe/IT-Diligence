...
export const RoleLevelPyramid = () => {
  ...
  const pyramidData = roleLevels.map((role) => {
    ...
    const avgTenure =
      count > 0
        ? (
            employeesInRole.reduce((sum, e) => sum + e.tenureYears, 0) / count
          ).toFixed(1)
        : "0.0";
    ...
    const avgExperience =
      count > 0
        ? Math.round(
            employeesInRole.reduce(
              (sum, e) => sum + e.totalExperienceYears,
              0
            ) / count
          )
        : 0;
    ...
    const avgStability =
      count > 0
        ? Math.round(
            employeesInRole.reduce((sum, e) => sum + e.stageStability, 0) /
              count
          )
        : 0;
      ...
      return (
        <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-lg">
          ...
          <div className="space-y-1 text-sm text-gray-600">
            ...
            <p>
              <span className="font-medium">Avg Tenure:</span> {data.avgTenure}{" "}
              years
            </p>
            <p>
              <span className="font-medium">Avg Experience:</span>{" "}
              {data.avgExperience} years
            </p>
            <p>
  ...
  const calculatePyramidHealth = (): {
    ...
    const juniorCount =
      pyramidData.find((d) => d.role === "Junior")?.count || 0;
    const midCount = pyramidData.find((d) => d.role === "Mid")?.count || 0;
    const seniorCount =
      pyramidData.find((d) => d.role === "Senior")?.count || 0;
    const leadCount = pyramidData.find((d) => d.role === "Lead")?.count || 0;
    const execCount =
      pyramidData.find((d) => d.role === "Executive")?.count || 0;
      ...
      <Card>
        <CardHeader>
          <div className="flex items-start justify-between">
            <div>
              ...
              <CardDescription>
                Role level distribution and structure health (n=250)
              </CardDescription>
            </div>
        ...
        <CardContent>
          ...
          <motion.div
            ...
            style={{
              borderColor: pyramidHealth.color,
              backgroundColor: `${pyramidHealth.color}15`,
            }}
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-3xl font-bold text-gray-900">
                  {pyramidHealth.score}/100
                </p>
                <p
                  className="mt-1 text-sm"
                  style={{ color: pyramidHealth.color }}
                >
            ...
            >
              ...
              >
                {pyramidData.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={ROLE_COLORS[entry.role as RoleLevel]}
                  />
                ))}
              </Bar>
            </BarChart>
              ...
              .map((data, index) => (
                ...
                >
                  ...
                  <div
                    ...
                    style={{ borderColor: ROLE_COLORS[data.role as RoleLevel] }}
                  >
          ...
          >
            ...
            <div className="flex items-center justify-between">
              {["Junior", "Mid", "Senior", "Lead", "Executive"].map(
                (role, index) => (
                  <div key={role} className="flex items-center">
                    <div className="text-center">
                      <div
                        ...
                        style={{
                          backgroundColor: ROLE_COLORS[role as RoleLevel],
                        }}
                      >
          ...
          >
            ...
            <ul className="mt-2 space-y-1 text-gray-600">
              <li>
                • A healthy pyramid has more junior employees than senior
                (supports growth)
              </li>
              <li>
                • Top-heavy organizations may face succession planning
                challenges
              </li>
              <li>
                • Bottom-heavy structures indicate strong pipeline but may lack
                leadership depth
              </li>
              <li>
                • Stage clarity and stability patterns may vary across role
                levels
              </li>
            </ul>
          </motion.div>
