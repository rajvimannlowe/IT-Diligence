...
export const EmployeeStageQuadrant = () => {
  ...
  const scatterData = MOCK_EMPLOYEES.map((emp) => {
    ...
    const xPosition =
      ...
      emp.stageDistribution.honeymoon +
      50;
  ...
  const getLegendItems = (): { label: string; color: string }[] => {
    switch (selectedDimension) {
      case "age":
        return Object.keys(AGE_COLORS).map((key) => ({
          label: key,
      ...
      case "gender":
        return Object.keys(GENDER_COLORS).map((key) => ({
          label: key,
          color: GENDER_COLORS[key],
        }));
      case "totalExperience":
        return Object.keys(EXP_COLORS).map((key) => ({
          label: key,
          color: EXP_COLORS[key],
        }));
      case "tenure":
        return Object.keys(TENURE_COLORS).map((key) => ({
          label: key,
          color: TENURE_COLORS[key],
        }));
      default:
        return [];
    }
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-lg">
          ...
          <div className="space-y-1 text-sm text-gray-600">
            ...
            <p>
              ...
              {employee.totalExperienceYears}yr ({employee.experienceGroup})
            </p>
            <p>
              <span className="font-medium">Tenure:</span>{" "}
              {employee.tenureYears.toFixed(1)}yr ({employee.tenureGroup})
            </p>
            <p
            ...
            <div className="mt-1 text-xs">
              <span style={{ color: STAGES.honeymoon.color.main }}>
                ■{employee.stageDistribution.honeymoon}%{" "}
              </span>
              <span style={{ color: STAGES["self-reflection"].color.main }}>
                ▲{employee.stageDistribution["self-reflection"]}%{" "}
              </span>
              <span style={{ color: STAGES["soul-searching"].color.main }}>
                ●{employee.stageDistribution["soul-searching"]}%{" "}
              </span>
              <span style={{ color: STAGES["steady-state"].color.main }}>
                ♦{employee.stageDistribution["steady-state"]}%
              </span>
            </div>
          </div>
        </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-start justify-between">
            <div>
              <CardTitle>
                4-Stage Employee Quadrant - Multi-Dimensional View
              </CardTitle>
              <CardDescription>
                Explore how employees cluster by stage distribution, colored by
                demographics (n={MOCK_EMPLOYEES.length})
              </CardDescription>
            </div>
        ...
        <CardContent>
          ...
          >
            <div className="rounded-lg border border-blue-300 bg-blue-50 p-3 text-center">
              ...
              <p className="mt-1 text-xl font-bold text-blue-700">
                {quadrantStats.topLeft}
              </p>
              <p className="text-xs text-blue-600">
                Honeymoon + Self-Reflection
              </p>
            </div>
            <div className="rounded-lg border border-green-300 bg-green-50 p-3 text-center">
              ...
              <p className="mt-1 text-xl font-bold text-green-700">
                {quadrantStats.topRight}
              </p>
              <p className="text-xs text-green-600">
                Steady-State + Self-Reflection
              </p>
            </div>
            <div className="rounded-lg border border-orange-300 bg-orange-50 p-3 text-center">
              ...
              <p className="mt-1 text-xl font-bold text-orange-700">
                {quadrantStats.bottomLeft}
              </p>
              <p className="text-xs text-orange-600">
                Honeymoon + Soul-Searching
              </p>
            </div>
            <div className="rounded-lg border border-purple-300 bg-purple-50 p-3 text-center">
              ...
              <p className="mt-1 text-xl font-bold text-purple-700">
                {quadrantStats.bottomRight}
              </p>
              <p className="text-xs text-purple-600">
                Steady-State + Soul-Searching
              </p>
            </div>
          </motion.div>
          ...
          >
            ...
            <ul className="mt-2 space-y-1 text-gray-600">
              <li>
                • <span className="font-medium">Position</span> shows where each
                employee sits based on their 4-stage distribution
              </li>
              <li>
                • <span className="font-medium">Color</span> reveals the
                selected demographic dimension (age, department, gender, etc.)
              </li>
              <li>
                • Toggle between dimensions to uncover insights like:
                <span className="italic">
                  {" "}
                  "50+ year olds with 80%+ steady-state vs 35-50 year olds with
                  55% steady-state"
                </span>
              </li>
              <li>
                • Hover over any employee to see their complete profile and
                stage breakdown
              </li>
            </ul>
          </motion.div>
